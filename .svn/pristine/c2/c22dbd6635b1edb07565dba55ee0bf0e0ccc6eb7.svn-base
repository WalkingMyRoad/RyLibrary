...
public class FloatWindowSmallView extends LinearLayout{
    ...
    public FloatWindowSmallView(Context context) {
        ...
        View viewMain = LayoutInflater.from(context).inflate(R.layout.view_float_small, this);
        View view = findViewById(R.id.small_window_layout);
        viewWidth = view.getLayoutParams().width;
        viewHeight = view.getLayoutParams().height;
        Button btnOpen = (Button) findViewById(R.id.open_float);
            ...
            public boolean onTouch(View view, MotionEvent motionEvent) {
                switch (motionEvent.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        if (Math.abs(xDownInScreen - xInScreen)<1
                                && Math.abs(yDownInScreen - yInScreen)<1) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                RyApplication.getLog().d("点击的距离xInView= "+xInView+"点击的距离xInScreen= "+xInScreen);
                break;
            case MotionEvent.ACTION_MOVE:
                xInScreen = event.getRawX();
                yInScreen = event.getRawY() - getStatusBarHeight();
            ...
            case MotionEvent.ACTION_UP:
                // 如果手指离开屏幕时，xDownInScreen和xInScreen相等，且yDownInScreen和yInScreen相等，则视为触发了单击事件。
                RyApplication.getLog().d("点击的距离x= "+Math.abs(xDownInScreen - xInScreen)+"点击的距离y= "+Math.abs(yDownInScreen - yInScreen));
