                ...
                public void run() {
                    ...
                    try{
                        ...
                        if(pointIndex >= mJiangJieBeenList.size()){
                            log.d("当前点位的标志位置 "+pointIndex);
                            JiangJieEnd();
                        }else if(mJiangJieBean.getNextPointSpeak()!=null && !mJiangJieBean.getNextPointSpeak().isEmpty()){//存在说话的内容
                            log.d("当前点位存在去下一个点要说的话 "+mJiangJieBean.getNextPointSpeak());
                            ...
                            mJiangJieBean.setArrivalPoint(false);
                        }else{
                            log.d("当前点位不存在去下一个点位说的话 ");
                            mJiangJieBean = mJiangJieBeenList.get(pointIndex);
                            mJiangJieBean.setArrivalPoint(false);
                            ChassisMove();
                        }
...
//                                        jiangJieBean.setPointInstructions();
        ...
        try {
            ...
            if(isComplete){
                if(mJiangJieBean.isArrivalPoint()){//是否到达下一个点位了
                    log.d("底盘是到达点位了 播发再次移动前的语音");
                    ImageClose();
                    AiuiAction();
                }else{
                    log.d("底盘没有到达点位了 底盘移动");
                    ChassisMove();
                }
            }else{
                JiangJieEnd();
    ...
    private void ChassisMove(){
        log.d("底盘移动开始了 去的点位 "+mJiangJieBean.getPointName());
